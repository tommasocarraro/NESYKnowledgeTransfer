from nesy.loaders import DataLoader, LTNDataLoader, DataLoaderFM, LTNDataLoaderFM
...
def nesy_tuning(seed, tune_config, train_set, val_set, n_users, n_items, item_genres_matrix, user_genres_matrix,
    ...
    """
    It performs the hyper-parameter tuning of NESY_MF. It can be using for tuning LTN_MF too.
...
def nesy_tuning_fm(seed, tune_config, train_set, genres_tr, val_set, genres_val, genres_all, n_users, n_items, n_genres,
                   n_side_feat, item_genres_matrix, user_genres_matrix, metric, sweep_id=None, exp_name=None):
    """
    It performs the hyper-parameter tuning of NESY_FM. This can be used for the tuning of LTN_FM too.
    ...
    def tune():
        with wandb.init():
            ...
            fm = FactorizationMachineModel([n_users, n_items, n_genres + 1], k, n_side_feat, normalize=True)
            optimizer = Adam(fm.parameters(), lr=lr, weight_decay=wd)
            trainer = LTNTrainerFM(fm, optimizer, u_g_matrix=user_genres_matrix, i_g_matrix=item_genres_matrix,
                                   p_sat_agg=p_sat_agg, p_forall=p_forall, p_exists=p_exists,
                                   wandb_train=True, p_pos=p_pos, p_neg=p_neg, threshold=0.5)
            trainer.train(train_loader, val_loader, metric, n_epochs=1000, early=10, verbose=1)
