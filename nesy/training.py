from nesy.loaders import LTNDataLoader, DataLoader, DataLoaderFM
...
def nesy_training(seed, train_set, val_set, n_users, n_items, item_genres_matrix, user_genres_matrix, config, metric,
    ...
    if test_loader is not None:
        ...
        return trainer.test(test_loader)  #, str(t)
...
def fm_training(seed, train_set, genres_tr, val_set, genres_val, n_users, n_items, n_genres, n_side_feat, config,
    ...
    set_seed(seed)
    # create train and validation loaders
    train_loader = DataLoaderFM(train_set, genres_tr, config["tr_batch_size"])
    val_loader = DataLoaderFM(val_set, genres_val, 256)
    # create model, optimizer, and trainer
    fm = FactorizationMachineModel([n_users, n_items, n_genres + 1], config["k"], n_side_feat)
    optimizer = Adam(fm.parameters(), lr=config["lr"], weight_decay=config["wd"])
